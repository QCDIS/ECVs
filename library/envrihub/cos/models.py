...
from envrihub.data_access.catalogue_metadata_client import (
    build_epos_catalogue_metadata_access,
)
from envrihub.data_access.models import (
    DataAccessObject,
    FileAccessObject,
    RESTfulAPIDataAccess,
)
...
class InvalidServiceDocumentationError(DistributionError):
    def __init__(self, message, resource_id, url):
        super().__init__(message)
        logging.error(
            f"Resource {resource_id} documentation at {url} cannot be used to create a data access client"
        )
...
class Distribution(CoSObject):
    ...
    def _build_dao(self):
        if self.type is None:
            raise NullDistributionTypeError(
                "Null type attribute in current Distribution", self.uid
            )
            ...
            if (base_url is not None) and (declared_parameters is not None):
                ...
                classname = self.metadata.get("serviceName").translate(
                    {ord(c): None for c in "/!@#*+-%&|[]()$ \r\n\t"}
                )
                content_type = None
                ...
                try:
                    self.dao = build_epos_catalogue_metadata_access(
                        base_url,
                        declared_parameters,
                        content_type,
                        self.description,
                    ...
                    logging.warning(ve)
                    logging.warning(
                        "Parameters names do not respect Python naming conventions, instantiating a generic REST client"
                    )
            ...
            # otherwise, it's OpenAPI
                ...
                try:
                    ...
                    raise InvalidServiceDocumentationError(
                        e, self.uid, self.service_documentation
                    )
        elif self.type == "DOWNLOADABLE_FILE":
            self.dao = FileAccessObject(self.metadata.get("downloadURL"))
        else:
            raise InvalidDistributionTypeError(
                "Null type attribute in current Distribution", self.uid, self.type
            )
