...
from pandas.core.dtypes.cast import (
    maybe_box_native,
)  # this is because of Scalars in dataframes...
...
def parse_response(
    response: requests.Response, expected_content_type: str | list = "json"
):
    """
...
def call_restful_api(
    ...
    if method in ["post", "POST"]:
        response = requests.post(
            url, params=params, data=payload, headers=headers, files=files
        )
    ...
    if response.status_code in [
        ...
        204,
        205,
    ]:  # the 20X status means 'success'
        return parse_response(response, expected_response_content_type)
    else:
        logging.error(
            "Request to " + str(url) + " failed with code " + str(response.status_code)
        )
