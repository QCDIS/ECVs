...
class Hub:
    """
    Encapsulates access to ENVRI Hub services.
    The only object you'll likely need to instantiate from the 'envrihub' package.
    """
    ...
    def search_catalogue(
        ...
        **kwargs
        ...
        """
        performs search on the Hub's catalogue.
        It returns an generator of Distribution items.

        Parameters
        ----------
        query:str
            free text query to be submitted to the Catalogue
        ...
        for i in self.catalogue.search(
            query=query,
            bbox=bbox,
    ...
    def fetch_from_catalogue(self, resource_id) -> Distribution:
        """Retrieves a specific resource from the catalogue, represented as a Distribution object.

        Parameters
        ----------
        resource_id:str
            the id of the resource you intend to pull.
